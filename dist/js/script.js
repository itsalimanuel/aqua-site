...
function changeService(navigate) {
    ...
    if(isNaN(Number(navigate))) {
        if(navigate == 'prev') {
            ...
            activeIndex = serviceContents.index($('section#services .service-item[data-hash="' + navigate + '"]'));
        }
    } else {
...
$(document).ready(function() {
    ...
    if(url.indexOf("services") !== -1) {
        var regextest = /#services-([a-zA-Z]+)/
        var found = url.match(regextest);
        if(found === null) {
            changeSection('services');
        } else {
            changeSection('services');
            changeService(found[1]);
        }
        console.log("1");
